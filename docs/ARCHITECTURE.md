...
See `android-twa/README.md` (and `.cursorrules` §1 / `AGENTS.md` for agent constraints). Bubblewrap is a **global** CLI (`npm run twa:ensure`); it is not a lockfile dependency. `/manifest.webmanifest` and `/.well-known` must stay public. Play Console listing remains a later epic.
